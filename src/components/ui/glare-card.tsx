...
export const GlareCard = ({
  ...
  const backgroundStyle = {
    ...
    "--shade":
      "radial-gradient( farthest-corner circle at var(--m-x) var(--m-y), rgba(0, 0, 0, 0.1) 12%, rgba(0, 0, 0, 0.15) 20%, rgba(0, 0, 0, 0.25) 120% ) var(--bg-x) var(--bg-y)/300% no-repeat", // Dark coffee shade
  ...
  return (
    ...
    >
      <div className="h-full grid will-change-transform origin-center transition-transform duration-[var(--duration)] ease-[var(--easing)] delay-[var(--delay)] [transform:rotateY(var(--r-x))_rotateX(var(--r-y))] rounded-[var(--radius)] border border-amber-500 hover:[--opacity:0.6] hover:[--duration:200ms] hover:[--easing:linear] hover:filter-none overflow-hidden">
        <div className="w-full h-full grid [grid-area:1/1] mix-blend-soft-light [clip-path:inset(0_0_0_0_round_var(--radius))]">
          <div className={cn("h-full w-full bg-[#130900]", className)}>
            {children}
          </div>
        </div>
